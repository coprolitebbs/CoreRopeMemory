...
//Вывод на 16битный сдвиговый регистр числа, кодирующего пины
...
//Считывание ниббла из 8 бит
uint8_t read_nibble8(unsigned int address) {
  uint8_t values[] = {0, 0, 0, 0, 0, 0, 0, 0};

  //Для точности можно увеличить количество проходов чтения, но, работает и так
  //for (uint8_t phase = 0; phase < 2; phase++)
  {
    //Разрядим
    discharge();
    //Подергаем нужным проводком по нужному адресу через сдвиговый регистр
    //Это нужно чтобы создать в ферритовых кольцах магнитный поток и зарядить конденсаторы
    //на датчиках тех колец, через которые проходит проводник с нужным адресом
  ...
  //Упакуем все считанные значения в один байт, где 0 - не считано ничего, 1 - считано что-то
  uint8_t result = 0;
  for (uint8_t ix = 0; ix < 8; ix++)
  {
    result = (result << 1) | ((values[ix] > 1) ? 1 : 0);
    //Serial.print(values[ix], DEC);
    //Serial.print(" ");
  }
  //Serial.println();
  return result;
...
void setup() {
  ...
  //Считаем и выведем нибблы по адресам 1..16
  for (int i = 0; i < 16; i++) {
    Serial.print("address[");
    if (i < 10) Serial.print("0");
    Serial.print( i, DEC);
    Serial.print("] = ");
    uint8_t nibble = read_nibble8((unsigned int)1 << i);
    Serial.print(nibble, DEC);
    Serial.print(" - ");
